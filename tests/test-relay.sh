...
if HOME="$TMP" env -u HANCHOU_AGENT_ID "$ROOT/bin/hanchou" --profile work relay emit \
...
if HOME="$TMP" HANCHOU_AGENT_ID=managed-worker "$ROOT/bin/hanchou" --profile work relay emit \
...
OUT="$(HOME="$TMP" HANCHOU_AGENT_ID=hch-test-implementer "$ROOT/bin/hanchou" --profile work relay emit \
  --type completed \
  --task hch-test \
  --execution exe-test \
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$EVENT_ID" --by orchestrator --json >"$TMP/pending-ack.out" 2>"$TMP/pending-ack.err"; then
  echo "expected unclaimed acknowledgement rejection" >&2
  exit 1
fi
grep -q 'must be claimed before acknowledgement' "$TMP/pending-ack.err"

if HOME="$TMP" env -u HANCHOU_AGENT_ID "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >"$TMP/unmanaged-claim.out" 2>"$TMP/unmanaged-claim.err"; then
...
if HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >"$TMP/wrong-claim.out" 2>"$TMP/wrong-claim.err"; then
...
if HOME="$TMP" HANCHOU_AGENT_ID=orchestrator "$ROOT/bin/hanchou" --profile work inbox ack "$EVENT_ID" --by another-agent --json >"$TMP/wrong-ack.out" 2>"$TMP/wrong-ack.err"; then
...
ESCAPE_ID='../../../../../../outside'
if HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox show "$ESCAPE_ID" >"$TMP/escape-show.out" 2>"$TMP/escape-show.err"; then
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox list --json >"$TMP/symlink-list.out" 2>"$TMP/symlink-list.err"; then
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox list --json >"$TMP/malformed-list.out" 2>"$TMP/malformed-list.err"; then
...
RECOVERABLE="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
RECOVERABLE_RECEIPT="$RELAY_ROOT/receipts/inbox-$RECOVERABLE_ID.json"
mkdir "$RECOVERABLE_RECEIPT"
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >"$TMP/receipt-blocked.out" 2>"$TMP/receipt-blocked.err"; then
...
if HOME="$TMP" HANCHOU_AGENT_ID=another-agent "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by another-agent --json >"$TMP/replay-wrong-actor.out" 2>"$TMP/replay-wrong-actor.err"; then
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >"$TMP/journal-blocked.out" 2>"$TMP/journal-blocked.err"; then
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >/dev/null
...
node -e 'const fs=require("fs"); const path=process.argv[1],id=process.argv[2]; const rows=fs.readFileSync(path,"utf8").trim().split("\n").map(JSON.parse); rows.find(row=>row.action==="acknowledged"&&row.event_id===id).by="tampered"; fs.writeFileSync(path,rows.map(JSON.stringify).join("\n")+"\n")' \
  "$RELAY_ROOT/journal.jsonl" "$RECOVERABLE_ID"
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >"$TMP/mismatched-journal.out" 2>"$TMP/mismatched-journal.err"; then
...
node -e 'const fs=require("fs"); const path=process.argv[1],id=process.argv[2]; const rows=fs.readFileSync(path,"utf8").trim().split("\n").map(JSON.parse); rows.push({...rows.find(row=>row.action==="acknowledged"&&row.event_id===id)}); fs.writeFileSync(path,rows.map(JSON.stringify).join("\n")+"\n")' \
  "$RELAY_ROOT/journal.jsonl" "$RECOVERABLE_ID"
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >"$TMP/duplicate-journal.out" 2>"$TMP/duplicate-journal.err"; then
...
node -e 'const fs=require("fs"); const path=process.argv[1],id=process.argv[2]; const rows=fs.readFileSync(path,"utf8").trim().split("\n").map(JSON.parse); const row=rows.find(row=>row.action==="acknowledged"&&row.event_id===id); delete row.schema; row.at="2026-01-01T00:00:00.000Z"; fs.writeFileSync(path,rows.map(JSON.stringify).join("\n")+"\n")' \
  "$RELAY_ROOT/journal.jsonl" "$RECOVERABLE_ID"
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >"$TMP/stripped-journal-schema.out" 2>"$TMP/stripped-journal-schema.err"; then
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RECOVERABLE_ID" --by orchestrator --json >"$TMP/mismatched-receipt.out" 2>"$TMP/mismatched-receipt.err"; then
...
LEGACY="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$LEGACY_ID" --by orchestrator --json >/dev/null
...
LEGACY_PENDING_RESIDUE="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$LEGACY_PENDING_RESIDUE_ID" --by orchestrator --json >/dev/null

LEGACY_PROCESSING_RESIDUE="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$LEGACY_PROCESSING_RESIDUE_ID" --by orchestrator --json >/dev/null
...
node -e 'const fs=require("fs"); const path=process.argv[1]; const row=JSON.parse(fs.readFileSync(path,"utf8")); row.dead_letter={at:"2026-01-01T00:00:00.000Z",reason:"v2.3.1 acknowledged residue"}; fs.writeFileSync(path,JSON.stringify(row,null,2)+"\n")' \
  "$RELAY_ROOT/inbox/acknowledged/$LEGACY_PROCESSING_RESIDUE_ID.json"
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$LEGACY_PROCESSING_RESIDUE_ID" --by orchestrator --json | node -e 'let text=""; process.stdin.on("data", chunk=>text+=chunk).on("end",()=>{const row=JSON.parse(text); if(row.already!==true) process.exit(1)})'
...
LEGACY_DEAD_CYCLES="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox dead-letter "$LEGACY_DEAD_CYCLES_ID" --reason ignored | grep -q "already dead-lettered $LEGACY_DEAD_CYCLES_ID"
...
node -e 'const fs=require("fs"); const rows=fs.readFileSync(process.argv[1],"utf8").trim().split("\n").map(JSON.parse); const canonical=rows.filter(row=>row.action==="dead-lettered"&&row.event_id===process.argv[2]&&row.schema==="hanchou.relay-terminal-journal.v1"&&row.retry_count===1); if(canonical.length!==1) throw new Error(`expected one migrated dead-letter journal entry, got ${canonical.length}`)' \
  "$RELAY_ROOT/journal.jsonl" "$LEGACY_DEAD_CYCLES_ID"
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox retry "$LEGACY_DEAD_CYCLES_ID" >/dev/null
test -f "$RELAY_ROOT/inbox/pending/$LEGACY_DEAD_CYCLES_ID.json"

LEGACY_STAGED_PENDING="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >"$TMP/legacy-staged-pending-claim.json"
...
LEGACY_STAGED_PROCESSING="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work relay recover | grep -q 'recovered 1 event(s)'
...
STAGED="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$STAGED_ID" --by orchestrator --json >/dev/null
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$STAGED_ID" --by orchestrator --json | node -e 'let text=""; process.stdin.on("data", chunk=>text+=chunk).on("end",()=>{const row=JSON.parse(text); if(row.already!==true) process.exit(1)})'

STAGED_RETRY="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$STAGED_RETRY_ID" --by orchestrator --json >/dev/null
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox retry "$STAGED_RETRY_ID" >"$TMP/staged-retry.out" 2>"$TMP/staged-retry.err"; then
...
STAGED_DEAD="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$STAGED_DEAD_ID" --by orchestrator --json >/dev/null
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox dead-letter "$STAGED_DEAD_ID" --reason test >"$TMP/staged-dead.out" 2>"$TMP/staged-dead.err"; then
...
PENDING_DEAD="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >"$TMP/pending-dead-claim.json"
...
PROCESSING_DEAD="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
RETRY_STAGED_DEAD="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox retry "$RETRY_STAGED_DEAD_ID" >"$TMP/retry-staged-dead.out" 2>"$TMP/retry-staged-dead.err"; then
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox retry "$RETRY_STAGED_DEAD_ID" >/dev/null
...
RETRY_ACK_CRASH="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=retry-ack-agent "$ROOT/bin/hanchou" --profile work inbox claim --to retry-ack-agent --json >/dev/null
...
if HOME="$TMP" HANCHOU_AGENT_ID=retry-ack-agent "$ROOT/bin/hanchou" --profile work inbox ack "$RETRY_ACK_CRASH_ID" --by retry-ack-agent --json >"$TMP/retry-ack-crash.out" 2>"$TMP/retry-ack-crash.err"; then
...
node -e 'const fs=require("fs"); const event=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); if(event.retry||event.retry_count!==1||event.lease?.claimed_by!=="retry-ack-agent") throw new Error("claim did not retire completed retry evidence")' \
  "$RELAY_ROOT/inbox/processing/$RETRY_ACK_CRASH_ID.json"
HOME="$TMP" HANCHOU_AGENT_ID=retry-ack-agent "$ROOT/bin/hanchou" --profile work inbox ack "$RETRY_ACK_CRASH_ID" --by retry-ack-agent --json >/dev/null

RETRY_ACK_CONFLICT="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=retry-conflict-agent "$ROOT/bin/hanchou" --profile work inbox claim --to retry-conflict-agent --json >/dev/null
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work relay recover >"$TMP/retry-ack-conflict.out" 2>"$TMP/retry-ack-conflict.err"; then
...
RETRY_RECOVER_CRASH="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=retry-recover-agent "$ROOT/bin/hanchou" --profile work inbox claim --to retry-recover-agent --json >/dev/null
node -e 'const fs=require("fs"); const path=process.argv[1]; const row=JSON.parse(fs.readFileSync(path,"utf8")); delete row.lease; row.retry_count=1; row.retry={at:new Date().toISOString(),from_state:"processing",retry_count:1,journal_schema:"hanchou.relay-inbox-transition-journal.v1"}; fs.writeFileSync(path,JSON.stringify(row,null,2)+"\n")' \
  "$RELAY_ROOT/inbox/processing/$RETRY_RECOVER_CRASH_ID.json"
HOME="$TMP" "$ROOT/bin/hanchou" --profile work relay recover | grep -q 'recovered 1 event(s)'
HOME="$TMP" "$ROOT/bin/hanchou" --profile work relay recover | grep -q 'recovered 0 event(s)'
...
RETRY_DEAD_CRASH="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=retry-dead-agent "$ROOT/bin/hanchou" --profile work inbox claim --to retry-dead-agent --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox dead-letter "$RETRY_DEAD_CRASH_ID" --reason original >/dev/null
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox dead-letter "$RETRY_DEAD_CRASH_ID" --reason replacement >"$TMP/retry-dead-crash.out" 2>"$TMP/retry-dead-crash.err"; then
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox retry "$RETRY_DEAD_CRASH_ID" | grep -q "already retried $RETRY_DEAD_CRASH_ID"

RETRY_PENDING_CRASH="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=retry-pending-agent "$ROOT/bin/hanchou" --profile work inbox claim --to retry-pending-agent --json >/dev/null
node -e 'const fs=require("fs"); const path=process.argv[1]; const row=JSON.parse(fs.readFileSync(path,"utf8")); delete row.lease; row.retry_count=1; row.retry={at:new Date().toISOString(),from_state:"processing",retry_count:1,journal_schema:"hanchou.relay-inbox-transition-journal.v1"}; fs.writeFileSync(path,JSON.stringify(row,null,2)+"\n")' \
...
HOME="$TMP" HANCHOU_AGENT_ID=retry-pending-agent "$ROOT/bin/hanchou" --profile work inbox ack "$RETRY_PENDING_CRASH_ID" --by retry-pending-agent --json >/dev/null

RECOVERY_CRASH="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=recovery-crash-agent "$ROOT/bin/hanchou" --profile work inbox claim --to recovery-crash-agent --json >/dev/null
...
NONFINITE="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=nonfinite-agent "$ROOT/bin/hanchou" --profile work inbox claim --to nonfinite-agent --json >/dev/null
...
EXPIRED="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
if HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$EXPIRED_ID" --by orchestrator --json >"$TMP/expired-ack.out" 2>"$TMP/expired-ack.err"; then
  echo "expected expired lease acknowledgement rejection" >&2
  exit 1
fi
grep -q 'event lease expired' "$TMP/expired-ack.err"
HOME="$TMP" "$ROOT/bin/hanchou" --profile work relay recover >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$EXPIRED_ID" --by orchestrator --json >/dev/null

RETRY="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
  --to-agent orchestrator --to-role orchestrator --delegation-depth 1 \
...
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox dead-letter "$RETRY_ID" --reason test >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox retry "$RETRY_ID" >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox show "$RETRY_ID" | node -e 'let text=""; process.stdin.on("data", chunk => text += chunk).on("end", () => { const row=JSON.parse(text); if (row.state !== "pending" || "dead_letter" in row.event) process.exit(1); })'
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox claim --to orchestrator --json >/dev/null
HOME="$TMP" "$ROOT/bin/hanchou" --profile work inbox ack "$RETRY_ID" --by orchestrator --json >/dev/null

RACE="$(HOME="$TMP" HANCHOU_AGENT_ID=worker "$ROOT/bin/hanchou" --profile work relay emit \
  --type checkpoint --from-agent worker --from-role worker \
...
HOME="$TMP" HANCHOU_AGENT_ID=race-agent "$ROOT/bin/hanchou" --profile work inbox claim --to race-agent --limit 1 --json >"$TMP/race-a.json" &
race_a=$!
HOME="$TMP" HANCHOU_AGENT_ID=race-agent "$ROOT/bin/hanchou" --profile work inbox claim --to race-agent --limit 1 --json >"$TMP/race-b.json" &
race_b=$!
wait "$race_a"
wait "$race_b"
...
HOME="$TMP" HANCHOU_AGENT_ID=race-agent "$ROOT/bin/hanchou" --profile work inbox ack "$RACE_ID" --by race-agent --json >/dev/null
