...
OUT="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind task_terminal \
  --task hch-test \
  --policy on_terminal \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$DELIVERY_ID" --by orchestrator --message 'Completed.' >/dev/null
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$DELIVERY_ID" --adapter local-session >/dev/null
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery show "$DELIVERY_ID" | node -e 'let text=""; process.stdin.on("data", chunk => text += chunk).on("end", () => { if (JSON.parse(text).state !== "delivered") process.exit(1); })'

RELAY_ROOT="$TMP/.local/share/hanchou/work/relay"
RECOVERABLE="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$RECOVERABLE_ID" --by orchestrator --message 'Recoverable.' >/dev/null
RECOVERABLE_RECEIPT="$RELAY_ROOT/receipts/delivery-$RECOVERABLE_ID.json"
mkdir "$RECOVERABLE_RECEIPT"
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter local-session >"$TMP/delivery-receipt-blocked.out" 2>"$TMP/delivery-receipt-blocked.err"; then
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter ignored-on-replay >"$TMP/delivery-journal-blocked.out" 2>"$TMP/delivery-journal-blocked.err"; then
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter ignored-on-replay >/dev/null
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter ignored-again >/dev/null
...
node -e 'const fs=require("fs"); const path=process.argv[1],id=process.argv[2]; const rows=fs.readFileSync(path,"utf8").trim().split("\n").map(JSON.parse); rows.find(row=>row.action==="delivery-delivered"&&row.delivery_id===id).adapter="tampered"; fs.writeFileSync(path,rows.map(JSON.stringify).join("\n")+"\n")' \
  "$RELAY_ROOT/journal.jsonl" "$RECOVERABLE_ID"
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter ignored >"$TMP/mismatched-journal.out" 2>"$TMP/mismatched-journal.err"; then
...
node -e 'const fs=require("fs"); const path=process.argv[1],id=process.argv[2]; const rows=fs.readFileSync(path,"utf8").trim().split("\n").map(JSON.parse); rows.push({...rows.find(row=>row.action==="delivery-delivered"&&row.delivery_id===id)}); fs.writeFileSync(path,rows.map(JSON.stringify).join("\n")+"\n")' \
  "$RELAY_ROOT/journal.jsonl" "$RECOVERABLE_ID"
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter ignored >"$TMP/duplicate-journal.out" 2>"$TMP/duplicate-journal.err"; then
...
node -e 'const fs=require("fs"); const path=process.argv[1],id=process.argv[2]; const rows=fs.readFileSync(path,"utf8").trim().split("\n").map(JSON.parse); const row=rows.find(row=>row.action==="delivery-delivered"&&row.delivery_id===id); delete row.schema; row.at="2026-01-01T00:00:00.000Z"; fs.writeFileSync(path,rows.map(JSON.stringify).join("\n")+"\n")' \
  "$RELAY_ROOT/journal.jsonl" "$RECOVERABLE_ID"
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$RECOVERABLE_ID" --adapter ignored >"$TMP/stripped-journal-schema.out" 2>"$TMP/stripped-journal-schema.err"; then
...
LEGACY="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$LEGACY_ID" --by orchestrator --message 'Legacy.' >/dev/null
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$LEGACY_ID" --adapter local-session >/dev/null
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$LEGACY_ID" --adapter ignored >/dev/null

STAGED="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_ID" --by orchestrator --message 'Staged.' >/dev/null
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$STAGED_ID" --adapter local-session >/dev/null
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$STAGED_ID" --adapter ignored | grep -q "recovered delivered $STAGED_ID"
test -f "$RELAY_ROOT/deliveries/delivered/$STAGED_ID.json"
test -f "$RELAY_ROOT/receipts/delivery-$STAGED_ID.json"

STAGED_RENDER="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_RENDER_ID" --by orchestrator --message 'Staged render invariant.' >/dev/null
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$STAGED_RENDER_ID" --adapter local-session >/dev/null
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_RENDER_ID" --by orchestrator --message ignored >"$TMP/staged-render.out" 2>"$TMP/staged-render.err"; then
...
STAGED_FAIL="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_FAIL_ID" --by orchestrator --message 'Staged fail invariant.' >/dev/null
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$STAGED_FAIL_ID" --adapter local-session >/dev/null
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery fail "$STAGED_FAIL_ID" --reason test >"$TMP/staged-fail.out" 2>"$TMP/staged-fail.err"; then
...
STAGED_RENDER_SOURCE="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery list --state pending --json >"$TMP/staged-render-list.json"
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_RENDER_SOURCE_ID" --by replacement --message 'Replacement message' | grep -q "already rendered $STAGED_RENDER_SOURCE_ID"
...
STAGED_FAILED_PENDING="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_FAILED_PENDING_ID" --by orchestrator --message ignored >"$TMP/staged-failed-render.out" 2>"$TMP/staged-failed-render.err"; then
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery fail "$STAGED_FAILED_PENDING_ID" --reason ignored | grep -q "already failed $STAGED_FAILED_PENDING_ID"
...
node -e 'const fs=require("fs"); const path=process.argv[1]; const row=JSON.parse(fs.readFileSync(path,"utf8")); row.retry={at:"2026-08-30T00:02:00.000Z",attempts:1,journal_schema:"hanchou.relay-terminal-journal.v1"}; fs.writeFileSync(path,JSON.stringify(row,null,2)+"\n")' \
  "$RELAY_ROOT/deliveries/failed/$STAGED_FAILED_PENDING_ID.json"
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery retry "$STAGED_FAILED_PENDING_ID" | grep -q "recovered retried $STAGED_FAILED_PENDING_ID"
node -e 'const fs=require("fs"); const row=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); if(row.status!=="pending"||row.attempts!==1||row.failure||row.retry?.attempts!==1) throw new Error("staged retry did not reach pending exactly once")' \
  "$RELAY_ROOT/deliveries/pending/$STAGED_FAILED_PENDING_ID.json"
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery retry "$STAGED_FAILED_PENDING_ID" | grep -q "already retried $STAGED_FAILED_PENDING_ID"
...
RETRY_FAIL_RACE="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery fail "$RETRY_FAIL_RACE_ID" --reason 'first failure' >/dev/null
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery fail "$RETRY_FAIL_RACE_ID" --reason 'second failure' >"$TMP/retry-fail-race.out" 2>"$TMP/retry-fail-race.err"; then
...
STAGED_FAILED_RENDERED="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$STAGED_FAILED_RENDERED_ID" --by orchestrator --message 'Rendered before failure.' >/dev/null
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$STAGED_FAILED_RENDERED_ID" --adapter forbidden >"$TMP/staged-failed-deliver.out" 2>"$TMP/staged-failed-deliver.err"; then
...
LEGACY_STAGED_RETRY="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery fail "$LEGACY_STAGED_RETRY_ID" --reason 'legacy retry' >/dev/null
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery list --state pending --json >"$TMP/legacy-staged-retry-list.json"
...
LEGACY_PENDING_RETRY="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery fail "$LEGACY_PENDING_RETRY_ID" --reason 'legacy pending retry' >/dev/null
node -e 'const fs=require("fs"); const path=process.argv[1]; const row=JSON.parse(fs.readFileSync(path,"utf8")); row.status="pending"; delete row.failure; delete row.retry; fs.writeFileSync(path,JSON.stringify(row,null,2)+"\n")' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery retry "$LEGACY_PENDING_RETRY_ID" | grep -q "already retried $LEGACY_PENDING_RETRY_ID"
...
DIRECT_RENDERED_RETRY="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$DIRECT_RENDERED_RETRY_ID" --by orchestrator --message 'Before staged failure.' >/dev/null
...
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery retry "$DIRECT_RENDERED_RETRY_ID" | grep -q "retried $DIRECT_RENDERED_RETRY_ID"
...
FRESH_RETRY="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery retry "$FRESH_RETRY_ID" >"$TMP/fresh-retry.out" 2>"$TMP/fresh-retry.err"; then
...
INCONSISTENT_FAILURE="$(HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
  --kind manual --policy always --renderer orchestrator \
  --destination '{"type":"local_session","agent":"orchestrator"}' \
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery list --json >"$TMP/inconsistent-failure-list.out" 2>"$TMP/inconsistent-failure-list.err"; then
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-rendered "$INCONSISTENT_FAILURE_ID" --by orchestrator --message ignored >"$TMP/inconsistent-failure-render.out" 2>"$TMP/inconsistent-failure-render.err"; then
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery mark-delivered "$INCONSISTENT_FAILURE_ID" --adapter forbidden >"$TMP/inconsistent-failure-deliver.out" 2>"$TMP/inconsistent-failure-deliver.err"; then
...
ESCAPE_ID='../../../../../../outside'
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery show "$ESCAPE_ID" >"$TMP/escape-show.out" 2>"$TMP/escape-show.err"; then
...
RACE_ID="dly_race"
HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery create \
...
if HOME="$TMP" "${HANCHOU_TEST[@]}" --profile work delivery list --json >"$TMP/symlink-list.out" 2>"$TMP/symlink-list.err"; then
